...
Ext.define('DshBrd.controller.TrkAtcCtl', {
    ...
    SlcChg: function(tablepanel, selections, options) {
        if (selections[0]) {
            this.getTrkAtc().down('form').getForm().loadRecord(selections[0]);
        }
    },
    ...
    FleDrpSve: function(button, e, options) {
    ...
    FleDrpCnl: function(button, e, options) {
        this.FleTmpClr(button.up('window'));
        this.getFleDrpInfStore().removeAll(true);
    },

    FleDrpEdt: function(editor, e, options) {
        Rcd = e.record;
        Ext.Ajax.request({
            url: '../shared/FleDrp.php',
            scope: this,
    ...
    TrkAtcDlt: function(button, e, options) {
    ...
    TrkAtcSve: function(button, e, options) {
